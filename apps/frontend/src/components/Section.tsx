...
function Section({section,issues,createIssue, deleteIssue}:SectionProps){
    ...
    return ( <div className="w-72 shrink-0 rounded-2xl border border-violet-400/20 bg-slate-800/95 p-4 shadow-xl shadow-black/30 backdrop-blur-sm">
        <div className= "mb-4 flex items-center justify-between">
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-wider text-slate-200">{section.title}</h2>
          <span className="rounded-xl bg-slate-950/70 px-2 py-1 text-xs font-medium text-violet-200">{issues.filter((issue)=>issue.sectionId===section.id).length}</span>
    </div>
          {issues
